...
jQuery(function() {
	...
	.on("submit", "#comment-form", function() {
		...
		var e = $(this),
            ...
            author = e.find("input[name=nickName]").val(),
            ...
            country = e.find("input[name=country]").val(),
            province = e.find("input[name=province]").val(),
            city = e.find("input[name=city]").val(),
            images = e.find("img[name=images]").val(),
	...
	*/
	...
	.on("click", "[data-action=like]", function(){
		...
		if(!e.hasClass("liked")){
			$.ajax( {
				url: rootDir + "ajax/like-comments.php",
